package com.wangwenjun.concurrency.second.concurrent.chapter1;
...
public class SingletonObject6 {
    ...
    //1.遇到new、getstatic、setstatic或者invikestatic这4个字节码指令时，
    // 对应的java代码场景为：new一个关键字或者一个实例化对象时、读取或设置一个静态字段时(final修饰、已在编译期把结果放入常量池的除外)、调用一个类的静态方法时。
    ...
    // 取的都是同一个INSTANCE对象，而不用去重新创建。当getInstance()方法被调用时，
    // SingleTonHoler才在SingleTon的运行时常量池里，把符号引用替换为直接引用，
